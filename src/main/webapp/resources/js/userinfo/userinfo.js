$(function() {
	setProperty();
	$("#searchUserByParam").bind("click", function () {
...
function setProperty(){
	...
	$('#table').bootstrapTable({
         ...
         columns: [
        	     {field: 'username', title: '登录名', sortable: false, halign: 'center'},
        	     {field: 'realname', title: '用户名', sortable: true, halign: 'center'},
        	     {field: 'password', title: '用户密码', sortable: false, halign: 'center'},
        	     {field: 'email', title: '邮箱', sortable: false, halign: 'center'},
        	     {field: 'phone', title: '电话', sortable: false, halign: 'center'},
        	     {field: 'enabled', title: '状态', sortable: false, halign: 'center'},
        	     {field: 'islocked', title: '是否被锁', sortable: false, halign: 'center'},
        	     {field: 'url', title: '个人链接', sortable: false, halign: 'center'},
        	     {field: 'register', title: '注册时间', sortable: false, halign: 'center'},
...
function addUser(){
	layer.open({
		  ...
		  content:'<form role="form" id="addinterfacetable"><table class="table border  text-center" id="sampleTable">'+
			  '<tr><td>登录名</td><td><input name="id" id="interfaceid" class="form-control form-control-lg" /></td><td>用户名</td><td><input name="sysid" id="sysid" class="form-control form-control-lg" >'+
            '</td></tr>'+
			  '<tr><td>登录密码</td><td><input name="name" id="name" class="form-control form-control-lg" /></td><td>电话号码</td><td><input name="port" id="port" class="form-control form-control-lg" /></td></tr>'+
			  '<tr><td>用户邮箱</td><td><input name="type" id="type" class="form-control form-control-lg" >'+
            '</td>'+
			  ' </td><td>状态</td><td><select name="stat" id="stat" class="form-control form-control-lg" >'+
            '        <option value="1">有效</option>'+
            '        <option value="0">无效</option>'+
            '</select></td></td></tr>'+
			  '<tr><td>角色</td><td><input name="timeout" id="timeout" class="form-control form-control-lg" /></td><td>个人链接</td><td><input name="method" id="method" class="form-control form-control-lg" /></td></tr>'+
			  '<tr><td>权限</td><td colspan="3"><div><input class="form-check-input" type="checkbox"  />增加</div><input class="form-check-input" type="checkbox"  />增加</td></tr>'+
	...
	$.ajax({
		...
		success:function(data){
			...
			layer.open({
	            ...
	            '</select></td></td></tr>'+	
				  '<tr> <td colspan="4"><button onclick="updateUserForm()" type="button" class="btn btn-primary">确定</button> <button type="reset"  class="btn btn-primary">重置</button></td></tr>'+
				  '</table></form>'
			});
		},
		error:function(){
